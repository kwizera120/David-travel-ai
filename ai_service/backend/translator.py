...
def _normalize_lang(code: str) -> str:
    # Handle common confusion between rw, ki, and kin
...
def _detect_by_hint(text: str) -> str | None:
    ...
    # Prioritize Kinyarwanda over Swahili
...
# Broad travel phrase overrides for Kinyarwanda (English -> Kinyarwanda)
# This ensures critical travel communication is accurate even if models fail.
KINYARWANDA_TRAVEL_PHRASES = {
    "hello": "Muraho",
    "how are you": "Amakuru?",
    "i am fine": "Ni meza",
    "thank you": "Urakoze",
    ...
    "go straight": "Genda imbere",
    "turn left": "Kata ibumoso",
    "turn right": "Kata iburyo",
    "i want to go to": "Ndashaka kujya i",
    "hospital": "Ibitaro",
    "hotel": "Hoteli",
    "market": "Isoko",
    "bus station": "Gare",
...
def _try_phrase_override(text: str, source_lang: str, target_lang: str) -> str | None:
    source = _normalize_lang(source_lang)
    target = _normalize_lang(target_lang)
    normalized_text = text.lower().strip().rstrip("?.!")

    if target == "rw" and source == "en":
        if normalized_text in KINYARWANDA_TRAVEL_PHRASES:
            return KINYARWANDA_TRAVEL_PHRASES[normalized_text]
        # Check if it's a simple "I want to go to [Place]"
        if normalized_text.startswith("i want to go to "):
            place = text[len("i want to go to "):].strip()
            return f"Ndashaka kujya {place}"
...
def translate_text(text: str, source_lang: str, target_lang: str) -> str:
    ...
    # Try phrase overrides first for accuracy in common travel scenarios
    override = _try_phrase_override(text, source, target)
    if override:
        return override
